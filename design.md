...
## 界面风格（2026-08-23 起为双风格）

两套**完整**界面并存，`html[data-skin]` 切换（导航栏按钮；站长默认在后台设置；访客 localStorage 覆盖；软导航后 `astro:after-swap` 恢复）：

| | 手帐 `journal`（默认） | 观星台 `observatory` |
|---|---|---|
| 字体 | 霞鹜文楷（手写感） | Noto Sans SC 900 + JetBrains Mono 注记 |
| 布局 | 顶部吸顶刊头 + 右上贴纸页签 | **左侧竖排导航栏，站名竖排书写**（移动端收回顶栏） |
| 色彩 | 米黄纸 + 砖红 | 深靛夜幕（昼=制图蓝本）+ 星光金 |
| 组件 | 纸片卡、微倾、软影、圆角 | 刻线蚀边、制图角标（hover 显形）、零圆角零阴影 |
| 装饰 | 和纸胶带、邮戳、波浪线 | § 章节计数、✦ 星点、# 标签前缀、细网格纸底 |

后台通过 `packages/admin/src/styles/observatory.css` 令牌级跟随（共用 localStorage `blog-skin`）。
教训记录：v1 皮肤（plain/ink/celadon）因只做 token 覆盖被评价为「换色」而移除；**新风格必须换字体世界 + 布局骨架 + 组件语言三件套才算成立**。深浅色（data-theme）两套风格各自支持。
